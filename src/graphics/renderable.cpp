...
namespace Graphics {
	Renderable::Renderable() {
		this->VAO.attach_buffer(OpenGL::Vertices, 0, NULL);
		this->VAO.attach_buffer(OpenGL::Elements, 0, NULL);
	...
	void Renderable::fill_buffers() {
		...
		this->VAO.fill_buffer(OpenGL::Vertices,
			this->get_vertex_buffer_size() * sizeof(Vertex),
			this->get_vertex_buffer());

		this->VAO.fill_buffer(OpenGL::Elements,
